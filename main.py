...
# message, x-co,y-co,width,height,inactive color,active color
def button(msg,x,y,w,h,ic,ac,action=None):
    #get_pos returns x and y coordinate in the form of a list
    mouse=pygame.mouse.get_pos()
    click=pygame.mouse.get_pressed()
    #activate color
    if x+w>mouse[0]>x and y+h>mouse[1]>y:
        pygame.draw.rect(gamedisplays,ac,(x,y,w,h))
    #0 th element is left click
...
def obstacle(obs_startx,obs_starty,obs):
    #to display different obstacles each time
    if obs==0:
...
def game_loop():
    ...
    obs=0
    y_change=0
    #any place excluding the background
    ...
    while not bumped:
        ...
        if x>display_width-(car_width+110) or x<110:
            crash()
        #after one car goes away from the screen other car
        if obs_starty>display_height:
            ...
            passed=passed+1
            score=passed*10
            if int(passed)%10==1 and int(passed)!=1:
                level=level+1
                obstacle_speed+2
                largetext=pygame.font.Font("freesansbold.ttf",80)
        ...
        if y<obs_starty+obs_height:
            #checking if obstacle and car touch each other
